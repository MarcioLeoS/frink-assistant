...
import { BookOpen, Folder, LayoutGrid, ChartBar, ChartPie, Copy, Tickets, BellRing, MapPin, Paperclip, TriangleAlert, Bot, Rocket, Settings, UsersRound, UserPen, Frame, PieChart, } from "lucide-react";
import { motion } from "framer-motion";
import AppLogo from "./app-logo";
import { BASE_URL } from "@/config/env";
import { SidebarGroupMenu } from "@/components/ui/sidebar-group-menu";
...
const mainNavItems: NavItem[] = [
    ...
    { title: "Clientes", url: `${base}/customers`, icon: UsersRound },
    ...
    { title: "Persona real", url: `${base}/person-request`, icon: UserPen },
    ...
    //{ title: "Documentos", url: `${base}/documents`, icon: Folder },
...
export function AppSidebar() {
    ...
    useEffect(() => {
        ...
        const currentIndex = mainNavItems.findIndex((item) => currentPath.startsWith(new URL(item.url).pathname));
        if (currentIndex !== -1) {
    ...
    return (
        <Sidebar collapsible="icon" variant="inset">
                ...
                <SidebarMenu>
                    {mainNavItems.map((item, index) => (
                        <SidebarMenuItem key={index} className="relative">
                            ...
                            {activeIndex === index && (
                                <motion.div
                                    className="absolute left-0 top-1/8 h-6 w-1 bg-white rounded-full"
            ...
            <SidebarGroupMenu projects={projects} />
            <SidebarFooter>
